...
class PlanetariumViewController: UIViewController {
    ...
    private func loadGridLines() {
        guard let anchor = sceneAnchor else { return }
        // Initialize grid with current camera FOV
        let currentFOV = cameraController.fieldOfView
        gridRenderer.updateGrid(for: currentFOV, anchor: anchor)
    }
    
    func updateGridForFOV(_ fov: Float) {
        guard let anchor = sceneAnchor else { return }
        gridRenderer.updateGrid(for: fov, anchor: anchor)
    }
